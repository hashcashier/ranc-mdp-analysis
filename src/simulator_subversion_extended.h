...
#ifndef POSH_MDP_ANALYSIS_SIMULATOR_SUBVERSION_EXTENDED_H
#define POSH_MDP_ANALYSIS_SIMULATOR_SUBVERSION_EXTENDED_H

#include "simulator.h"

pair<LD, LD> simulateSubversionExtended(unordered_map<State, pair<UI16, UI16>, StateHash> &stateChoiceMap, LD alpha, LD gamma, UI32 T, UI32 K, UI32 S, LD D, bool overpay = false, bool compareChoiceChar = false, bool forgive = false, UI64 duration =
1ULL << 20, UI64 seed = 31337) {
    ...
    for (UI32 i = 0; i < duration; i++) {
        ...
        State repState = getRepresentativeState(w_a, h, f, rgh, mch, K, false, !forgive, overpay);
...
//        bool canMatch = canWait and a >= h and h <= K and h > 0 and f == 1 and gamma > 0;
        ...
        if (choice == 0) { // adopt
            ...
            LD Vds = h < S
                     ? 0
                     : (h + 1 - S) * D;
            ra += alpha * (safeSum(w_a, 0, h+1)) + Vds;
            ra_t += alpha * (safeSum(w_a_t, 0, h+1)) + Vds/lambda;
...
//                f = (w_h.size() - 1) <= K and (w_a.size() - 1) >= (w_h.size() - 1);
                ...
                LD Vds = h < S
                         ? 0
                         : (h + 1 - S) * D;
                ra += alpha * (safeSum(w_a, 0, h)) + Vds;
                ra_t += alpha * (safeSum(w_a_t, 0, h)) + Vds/lambda;
                rh += (1 - alpha) * (safeSum(w_h, 0, K) + (gamma * mch));
                rh_t += (1 - alpha) * (safeSum(w_h_t, 0, K) + (gamma * mch_t));
                mch = 0;
                mch_t = 0;
...
//                f = (w_h.size() - 1) <= K and (w_a.size() - 1) >= (w_h.size() - 1);
    ...
    return make_pair(ra / duration, ra_t / (duration/lambda));
}

#endif //POSH_MDP_ANALYSIS_SIMULATOR_SUBVERSION_EXTENDED_H
