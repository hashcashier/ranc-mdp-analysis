//
// Created by rami on 19/03/2021.
//

#ifndef POSH_MDP_ANALYSIS_SIMULATOR_INCENTIVE_EXTENDED_H
#define POSH_MDP_ANALYSIS_SIMULATOR_INCENTIVE_EXTENDED_H

#include "simulator.h"

pair<LD, LD> simulateIncentiveExtended(unordered_map<State, pair<UI16, UI16>, StateHash> &stateChoiceMap, LD alpha, LD gamma, UI32 T, UI32 K, bool overpay = false, bool compareChoiceChar = false, bool forgive = false, bool estimate = false, UI64 duration =
1ULL << 20, UI64 seed = 31337) {
    ...
    for (UI32 i = 0; i < duration; i++) {
        ...
        State repState = getRepresentativeState(w_a, h, f, rgh, mch, K, true, !forgive, overpay, estimate);
...
//                f = (w_h.size() - 1) <= K and (w_a.size() - 1) >= (w_h.size() - 1);
    ...
    return make_pair(ra / (ra + rh), ra_t / (ra_t + rh_t));
}


#endif //POSH_MDP_ANALYSIS_SIMULATOR_INCENTIVE_EXTENDED_H
